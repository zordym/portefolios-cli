...
use commands::{list, new, open, run, docs};
...
fn main() -> Result<()> {
    ...
    // Execute commands
    match cli.command {
        Commands::List { language, architecture, status, format } => {
            list::execute(&config, language, architecture, status, &format)?;
        }
        Commands::New { name, language, architecture, description, interactive } => {
            new::execute(&config, &name, &language, &architecture, description.as_deref(), interactive)?;
        }
        Commands::Open { project, editor, terminal } => {
            open::execute(&config, &project, editor.as_deref(), terminal)?;
        }
        Commands::Run { project, port, profile, debug } => {
            run::execute(&config, &project, port, profile.as_deref(), debug)?;
        }
        Commands::Docs { output, format, detailed } => {
